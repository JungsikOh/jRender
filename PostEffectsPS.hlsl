...
float LinearizeDepth(float2 texcoord)
{
    ...
    posProj.z = shadowMaps[1].Sample(linearClampSampler, texcoord).r;
    posProj.w = 1.0;
...
float LinearizeDepthCube(float4 posModel)
{
    float near = 0.05;
    float far = 50.0;
    ...
    posProj.z = shadowCubeMap.Sample(shadowPointSampler, posModel.xyz).r;
...
float4 main(VSToPS input) : SV_Target0
{
    ...
    col = pow(col, float3(1.0 / gammaScale, 1.0 / gammaScale, 1.0 / gammaScale));
