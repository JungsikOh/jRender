...
// C++17에 도입된 파일 시스템 라이브러리
// path 객체를 인자로 받고 path로 파일/디렉토리 정보를 수집한다.
// path 클래스는 그냥 경로를 나타낼 뿐 실제 파일을 지칭하는 것은 아니다.
...
void ModelLoader::Load(std::string basePath, std::string filename,
    ...
    cout << pScene->mRootNode->mNumChildren << endl;
...
// UpdateNoramls()와 어느정도 비슷하게 흘러간다.
